...
uses
  ...
  Vcl.Graphics, Vcl.ComCtrls, Vcl.Clipbrd, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, LanguageManager;
...
begin
  Result := MessageDlg(Translate(sMessage), mtConfirmation, [mbYes, mbNo], 0) = mrYes;
end;

class procedure MessageService.ShowError(sMessage: string);
begin
  MessageDlg(Translate(sMessage), mtError, [mbOk], 0);
end;

class procedure MessageService.ShowMessage(sMessage: string);
begin
  MessageDlg(Translate(sMessage), mtInformation, [mbOk], 0);
end;

class procedure MessageService.ShowWarning(sMessage: string);
begin
  MessageDlg(Translate(sMessage), mtWarning, [mbOk], 0);
end;
