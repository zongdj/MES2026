  ...
  begin
    ...
    try
      ...
      begin
        ...
        if (PropInfo^.PropType^.Kind <> tkClass) and (PropInfo^.SetProc = nil)
        then
          Continue;
...
{$WARNINGS ON}
        if not IsPublishedProp(Source, strPropName) then
          Continue;
...
begin
  ...
  begin
    AMsg.Result := DefWindowProcW(FInterWindow, AMsg.Msg, AMsg.WParam,
      AMsg.LParam);
  end;
end;
